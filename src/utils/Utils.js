...
export function zipWithIndexTwice<A>(
  arr: Array<A>,
...
function useExponential(num: number): boolean {
  const absNum = Math.abs(num);
  return (
    absNum >= Configs.MinScientificNotation ||
    (absNum > 0 && absNum <= Configs.MaxScientificNotation)
  );
}
export function numberWithCommas(x: string, round: ?boolean = true) {
  const xNumber = Number(x);
  const xString =
    round && decimalPlaces(x) > Configs.MaxPrecision
      ? xNumber.toFixed(Configs.MaxPrecision)
      : x;
