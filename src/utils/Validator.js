...
export default class Validator {
  static getLastEffectiveOperator(queue: Array<Operation>): Operation {
    ...
    const filterOut = Array(
      ...
      OperationSubType.ParenthesisOpen,
    );
  ...
  static validOperator(op: Operation, queue: Array<Operation>): boolean {
    ...
    if (op.operationSubType === OperationSubType.UnaryOp) {
      return (
        !lastOp ||
        lastOp.operationSubType === OperationSubType.BinaryOp ||
        lastOp.operationSubType === OperationSubType.UnaryOp
      );
      ...
      return (
        lastOp &&
        (lastOp.operationType === OperationType.Constant ||
          lastOp.operationSubType === OperationSubType.BinaryOp)
      );
    } else {
      return false;
  ...
  static validDigit(digit: string, queue: Array<Operation>): boolean {
    ...
    if (
      lastEffectiveOp &&
      digit === DECIMAL &&
      lastEffectiveOp.operationType === OperationType.Constant
    ) {
      return lastEffectiveOp.stringVal.indexOf(DECIMAL) === -1;
    } else if (
      lastOp &&
      lastOp.operationSubType === OperationSubType.BackwardUnaryOp
    ) {
      return false;
    } else if (
      lastOp &&
      lastOp.operationSubType === OperationSubType.ParenthesisClose
    ) {
      return false;
    } else {
      return true;
    ...
    if (op.operationSubType === OperationSubType.ParenthesisOpen) {
      return (
        ...
        isInArray(
          ...
          Array(OperationSubType.BinaryOp, OperationSubType.UnaryOp),
        )
      );
    } else if (op.operationSubType === OperationSubType.ParenthesisClose) {
      if (lastOp && lastOp.operationType === OperationType.Constant) {
        const oParen: Array<Operation> = queue.filter(
          op => op.operationType == OperationType.Parenthesis,
        );
        const oParenPriority: number =
          oParen.length > 0
            ? oParen.reduce(function(a, b) {
                return a + b.priority;
              }, 0)
            : 0;
  ...
  static replaceOperator(op: Operation, queue: Array<Operation>) {
    ...
    return (
      lastOp &&
      (lastOp.operationSubType === op.operationSubType &&
        op.operationSubType === OperationSubType.BinaryOp)
    );
  }
}
