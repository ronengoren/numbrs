...
export default class CalculatorUtils {
  static getOperators(queue) {
    return queue.filter(op =>
      isInArray(op.operationType, Array(OperationType.Operation)),
    );
  }
  ...
  static getOperatorsWithParenthesis(queue) {
    return queue.filter(op =>
      isInArray(
        op.operationType,
        Array(OperationType.Operation, OperationType.Parenthesis),
      ),
    );
  }
  ...
  static getLastNumber(queue) {
    return (
      queue.filter(op => op.operationType == OperationType.Constant)[0] || 0
    );
  }
}
