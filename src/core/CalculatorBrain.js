...
export default class CalculatorBrain {
  ...
  setOperand(op: Operation): ?Operation {
    ...
    if (
      ...
      (this.cleared ||
        ...
        (op.operationArgs.has(OperationArgs.NotParseable) &&
  ...
  getResult(): string {
    const numbers: Array<Operation> = this.queue.filter(
      op => op.operationType === OperationType.Constant,
    );
    const operatorsWithParenthesis = CalcUtils.getOperatorsWithParenthesis(
      this.queue,
    );
  ...
  evaluateParenthesis(
    operators: Array<Operation>,
    parenthesisPriority: number = 0,
  ): Array<Operation> {
    ...
    const remaining: Array<Operation> = this.evaluateParenthesis(
      remainingOps,
      parenthesisPriority,
    );
  ...
  evaluate(
    ...
    let maxPriority: [number, number, Operation] = zippedOps.reduce(function(
      a,
      b,
    ) {
    ...
    if (num1) {
      if (op.operationSubType === OperationSubType.BackwardUnaryOp) {
        ...
        isInArray(
          op.operationSubType,
          Array(OperationSubType.UnaryOp, OperationSubType.BackwardUnaryOp),
        )
      ) {
        acc = op.val(num1.val);
        tail = numbers.slice(numIdx + 1, numbers.length);
